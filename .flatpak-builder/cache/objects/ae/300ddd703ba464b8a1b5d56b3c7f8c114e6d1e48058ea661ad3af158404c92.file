/*
 * et-c-k5e1_err.c:
 * This file is automatically generated; please do not edit it.
 */
#if defined(_WIN32)
# include "win-mac.h"
#endif

#if !defined(_WIN32)
extern void initialize_k5e1_error_table (void);
#endif

#define N_(x) (x)

/* Lclint doesn't handle null annotations on arrays
   properly, so we need this typedef in each
   generated .c file.  */
/*@-redef@*/
typedef /*@null@*/ const char *ncptr;
/*@=redef@*/

static ncptr const text[] = {
	N_("Plugin does not support interface version"),
	N_("Invalid module specifier"),
	N_("Plugin module name not found"),
	N_("The KDC should discard this request"),
	N_("Can't create new subsidiary cache"),
	N_("Invalid keyring anchor name"),
	N_("Unknown keyring collection version"),
	N_("Invalid UID in persistent keyring name"),
	N_("Malformed reply from KCM daemon"),
	N_("Mach RPC error communicating with KCM daemon"),
	N_("KCM daemon reply too big"),
	N_("No KCM server found"),
	N_("Authorize and set hw-authent ticket flag"),
	N_("Set hw-authent ticket flag but do not authorize"),
    "mit-krb5", /* Text domain */
    0
};

#include <com_err.h>

const struct error_table et_k5e1_error_table = { text, -1750600192L, 14 };

#if !defined(_WIN32)
void initialize_k5e1_error_table (void)
    /*@modifies internalState@*/
{
    (void) add_error_table (&et_k5e1_error_table);
}
#endif
